...
// 2. Newton refinement: r = r0 * (1 + σ/2 + 3σ²/8) where σ = 1 - x*r0²
...
//   oddExp: unused for decimal (kept for API compatibility)
//
// Output:
//   Approximation of 1/sqrt(x) scaled by 10^7, range [3162278, 10000000]
//
// Design rationale (SoftFloat-aligned):
// - Working scale R ≈ 10^5/sqrt(x) chosen so sig*R² ≤ 10^17 (fits 64-bit)
// - sigma = (10^16 - sig*R²) / 10^16 computed as integer
...
inline constexpr std::uint32_t approx_recip_sqrt32(std::uint32_t sig, unsigned int /* oddExp */) noexcept
{
    ...
    // This ensures sig * R² ≤ 10^17, safely within 64-bit
    std::uint32_t R = static_cast<std::uint32_t>(r0_scaled / 100000000000ULL);
    
    // ---- Step 3: First Newton iteration ----
    // Target: sig * R² = 10^16 when R is accurate
    // sigma_16 = 10^16 - sig * R² (sigma scaled by 10^16)
    // R_new = R + R * sigma_16 / (2 * 10^16)
    
    constexpr std::uint64_t target16 = 10000000000000000ULL;  // 10^16
    
    std::uint64_t R_sq = static_cast<std::uint64_t>(R) * R;
    ...
    // Split to avoid overflow: (R * (sigma_16 >> 8)) >> (16 + 32 - 8) = (R * (sigma_16 >> 8)) >> 40
    // Simplified: correction = (R * sigma_16) / (2 * 10^16)
    //           = (R * (sigma_16 / 10^8)) / (2 * 10^8)
    std::int64_t sigma_8 = sigma_16 / 100000000LL;
    std::int64_t correction = (static_cast<std::int64_t>(R) * sigma_8) / 200000000LL;
    R = static_cast<std::uint32_t>(static_cast<std::int64_t>(R) + correction);
    
    // ---- Step 4: Second Newton iteration (SoftFloat uses σ² term, we use full iteration) ----
    R_sq = static_cast<std::uint64_t>(R) * R;
    ...
    correction = (static_cast<std::int64_t>(R) * sigma_8) / 200000000LL;
    R = static_cast<std::uint32_t>(static_cast<std::int64_t>(R) + correction);
    
    // ---- Step 5: Convert to output scale ----
    // R is 10^5/sqrt(x), output is 10^7/sqrt(x)
    std::uint32_t result = R * 100U;
...
// - When accurate: sig * r0² / 10^47 = 1, i.e., sig * r0² = 10^47
// - We compute y = (sig / 10^8) * (r0² / 10^24), target = 10^15
// - sigma_15 = 10^15 - y
...
inline constexpr std::uint64_t approx_recip_sqrt64(std::uint64_t sig, unsigned int /* oddExp */) noexcept
{
    ...
    // Newton: r_new = r + r * (1 - x*r²) / 2
    //
    // Working formula:
    // y = (sig / 10^8) * (r0² / 10^24) represents sig * r0² / 10^32
    // When r0 is accurate: y = 10^15 (since sig * r0² = 10^47 => /10^32 = 10^15)
    // sigma_15 = 10^15 - y
